...
const Form = ({ setBikes }) => {
  ...
  return (
    <form onSubmit={handleSubmit}>
      <input value={name} onChange={(e) => setName(e.target.value)} placeholder='Name' required='required' pattern='([a-z]|[A-Z]|[0-9]){5,30}' title='5 to 30 letters' />
      <input value={type} onChange={(e) => setType(e.target.value)} placeholder='Type' required='required' pattern='([a-z]|[A-Z]|[0-9]){5,30}' title='5 to 30 letters' />
      <br />
      ...
      <input value={size} onChange={(e) => setSize(e.target.value)} placeholder='Wheel size' required='required' pattern='[0-9]+\.[0-9]+|[0-9]+' title='only a number' />
      <br />
      <input value={price} onChange={(e) => setPrice(e.target.value)} placeholder='Price' required='required' pattern='[0-9]+\.[0-9]+|[0-9]+' title='only a number' />
